...
spa.chat = (function () {
  'use strict';
	...
	setJqueryMap = function () {
		var	$slider = stateMap.$container.find('.spa-chat');
	  jqueryMap = {
			$slider   : $slider,
	...
	setPxSizes = function () {
		var px_per_em, window_height_em, 
			window_distance_above_min, opened_height_em;
		...
		window_distance_above_min = 
			window_height_em - configMap.window_height_min_em;

		// set slider height relative to window size
		opened_height_em 
			= window_distance_above_min >= 0
			? configMap.slider_opened_em
			: configMap.slider_opened_em + window_distance_above_min;

		if ( opened_height_em < configMap.slider_opened_min_em ) {
  ...
  // End private DOM methods to manage chat message
	...
	onTapList = function(event) {
	  ...
	  var chatee_id = $tapped.attr('data-id');
	...
	onUpdatechat = function(event, msg_map) {
	  ...
	  var is_user = sender.get_is_user();
	...
	// Returns   : true
	// Throws    : 
	//   Javascript error object and stack trace on unacceptable
	...
	//   it with HTML content. It then initializes the elements,
	//   events, and handlers to provide the user with a chat-room
	//   interface.
	// Returns   : true on success, false on failure
	// Throws    : none
	initModule = function ( $append_target ) {
	  var $template = $('#spa-chat-template');
	  var context = { title: 'Chat' };
		var html = spa.util.compileTemplate($template, context);
	  ...
	  var $list_box = jqueryMap.$list_box;
	...
	//   otherwise animates to requested state
	// Returns   :
